...
const pagina = () => {
    ...
    const [invitadosN, setInvitadosN] = useState<number>(1)
    useEffect(() => {
    ...
    const deleteSubmit = async () => {
      ...
      const event2 = events?.sort((a , b) => a.titulo.localeCompare(b.titulo))

      //const event3 = events?.sort((a , b) => new Date(a.fecha).getTime() - new Date(b.fecha).getTime()).filter((e)=> e.invitados.length > 1)
      ...
      return (
          ...
          <input type="number" onChange={(e)=> setInvitadosN(parseInt(e.target.value))}></input>

          <h1>eventos</h1>
          {event2?.map((e)=> {

            if(e.invitados.length !== invitadosN) return
